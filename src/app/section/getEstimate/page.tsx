...
const EstimatePDFContent: React.FC<EstimatePDFContentProps> = ({ estimationResult }) => {
  return (
    <div className="p-8 font-sans text-gray-800 bg-white w-[800px]">
      {/* Top Section: Logo + Project Estimate */}
...
export function GetEstimateSection({
  ...
  <DialogContent
    className="sm:max-w-lg w-full max-h-[90vh] overflow-y-auto bg-secondary text-foreground p-0 rounded-lg"
  >
    {/* Scrollable Content Container */}
   <div className="p-6 space-y-6">
  <DialogHeader className="flex flex-col space-y-3"> {/* Reduced gap here */}
    {/* Logo + Title Row */}
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between w-full gap-4">
      ...
      <div className="text-left sm:text-right">
        <CardTitle className="text-2xl font-bold nav-text-gradient mb-1">
          Project Estimate
        </CardTitle>
      </div>
    </div>
    ...
    {serviceType && (
      <div className="w-full text-center space-y-1"> {/* Small gap between type and description */}
      ...
      {estimationResult && (
        <div className="space-y-4">
          ...
          <div className="flex justify-between items-center font-bold text-lg pt-4 bg-primary/10 p-3 rounded-md">
            <span className="nav-text-gradient">
              Total Estimated Timeline:
            </span>
            <span className="text-primary">
              {estimationResult.totalDuration}
            </span>
          </div>
        </div>
      )}
